...
class LLaMAModel:
    ...
    def _init_critic(self):
        ...
        if self.config.critic_peft_path is not None:
            lora_state_dict = torch.load(os.path.join(self.config.critic_peft_path, 'adapter_model.bin'))
            model.v_head.load_state_dict({
                    "summary.weight": lora_state_dict["v_head.summary.weight"],
                    "summary.bias": lora_state_dict["v_head.summary.bias"]
                })
        if self.config.critic_peft_path is not None:
            model = PeftModel.from_pretrained(model, self.config.critic_peft_path, adapter_name="default", is_trainable=True)
        else:
            model = get_peft_model(model, self.lora_config)
        model.to(self.device)
    ...
    def _limit_token(self):
        allowed_tokens = [str(i) for i in range(20)]  # 允许的token（需转换为token ID）
    ...
    def generate_act(self, prompts_ids, temperature):
        ...
        processor = ConstrainedLogitsProcessor(self.allowed_token_ids, prompt_length, 220) 
        # processor = ConstrainedLogitsProcessor1(self.allowed_token_ids, prompt_length) 
        ...
        # sequences_ids = torch.cat([sequences_ids, torch.tensor([[self.tokenizer.eos_token_id]])], dim=-1)
